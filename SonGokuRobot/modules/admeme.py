...
from SonGokuRobot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from SonGokuRobot.modules.helper_funcs.admin_rights import (
    user_can_pin,
    user_can_promote,
    user_can_changeinfo,
)
...
def promote(update, context):
    ...
    if user_can_promote(chat, user, context.bot.id) is False:
        message.reply_text("You don't have enough rights to promote someone! Nub nibba lel :P.")
        return ""
...
def demote(update, context):
    ...
    if user_member.status == "creator":
        message.reply_text("I'm not gonna demote Creator this group rn.. he is even stronger than me ＞﹏＜.")
        return ""

    if not user_member.status == "administrator":
        message.reply_text(
            "Ehhhh? How am I supposed to demote someone who is not even an admin!"
        )
        return ""
    ...
    try:
        ...
        message.reply_text("Successfully demoted! Better luck next time ;).")
        return (
            "<b>{}:</b>"
            "\n#DEMOTED"
            "\n<b>Admin:</b> {}"
