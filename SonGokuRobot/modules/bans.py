...
from SonGokuRobot.modules.helper_funcs.admin_rights import user_can_ban
...
def ban(update, context):
    ...
    try:
        ...
        context.bot.sendMessage(
            chat.id,
            "Ka..Me..Ha..Me..Haaaaaa! Admin {} has successfully killed {} in <b>{}</b>!.".format(
                mention_html(user.id, user.first_name),
                mention_html(member.user.id, member.user.first_name),
                html.escape(chat.title)
            ),
...
def temp_ban(update, context):
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text("How can I ban admins?.. are you crazy? DUMMY!")
        return ""
    ...
    try:
        ...
        message.reply_text("Ka..Me..Ha..Me..Haa!!!  Admin {} has successfully killed {} in <b>{}</b> for {}!".format(
                mention_html(user.id, user.first_name),
                mention_html(member.user.id, member.user.first_name),
...
def kick(update, context):
    ...
    if res:
        ...
        context.bot.sendMessage(
            chat.id,
            "**Ki blasts** Admin {} has successfully kicked {} in <b>{}</b>!".format(
            mention_html(user.id, user.first_name),
            mention_html(member.user.id, member.user.first_name),
            html.escape(chat.title)),
            parse_mode=ParseMode.HTML,
...
def banme(update, context):
    ...
    if res:
        update.effective_message.reply_text("Pahli fursat me nikal!")
        log = (
            "<b>{}:</b>"
            "\n#BANME"
            "\n<b>User:</b> {}"
